...
const getFeatureRequest = (layerName, pageSize, filter) => {
    ...
    let featureRequest = new ol.format.WFS().writeGetFeature(options);
 ...
 * @param type 查询类型
 * @param pageSize  分页数量
 * @param geometry 多边形geometry类型
 * @returns {Promise<unknown>}
 */
const spaceFilter = (map,url, type, pageSize, geometry) => {
    map.removeLayer(filterLayer);
    let source = new ol.source.Vector();
    let style = new ol.style.Style({
        stroke: new ol.style.Stroke({
            color: 'rgba(18,94,234,0.7)',
            width: 1,
        }),
        fill: new ol.style.Fill({
            color: 'rgba(128,225,255,0.20)',
        }),
    });
    source.addFeature(new ol.Feature({geometry: geometry}));
    ...
    layerNames.forEach(layerName => {
        let filter = ol.format.filter.intersects(GEOTYPE(layerName, 0), geometry, SRS_NAME)
...
const styleFunction = function (feature) {
    const image = new ol.style.Circle({
        ...
        stroke: new ol.style.Stroke({color: 'rgb(31,37,154)', width: 1}),
    });
    const styles = {
...
const addGeoJSON = (map, featureId) => {
    ...
    if (featureId === undefined || featureId === null || featureId === '') {
        ...
        let feature = new ol.format.GeoJSON().readFeatures(features);
