...
import com.example.webgistest.dao.EditMapper;
 ...
 */
@Service
public class ImplEditService implements IEditService {
    @Autowired
    EditMapper editMapper;

    public ServerResponse<List<Map<String,Object>>> getEdit(String name){
        List<Map<String,Object>> list=editMapper.getEdit(name);
        if (list.size() == 0) {
    ...
    public ServerResponse<Integer> addEdit(String name, String geom){
        int list = editMapper.addEdit(name, geom);
        if (list == 0) {
    ...
    public ServerResponse<Integer> updateEdit(int id, String name, String geom){
        int list = editMapper.updateEdit(id, name, geom);
        if (list == 0) {
